...
1. **Find what's out of sync.** Run the coverage report:
   ...
   ```bash
   ./scripts/translations/find-missing.sh --keys
   ```
...
3. **Translate** each missing key into every locale and add them to the payload
   `scripts/translations/payload.json`, as `key → { locale → translation }`. This file
   is a transient working file — it is gitignored and **must not be committed**; it is
...
4. **Sync.** Insert the translations and remove stale keys:
   ...
   ```bash
   python3 scripts/translations/sync.py --dry-run   # preview first
   python3 scripts/translations/sync.py             # apply
   ```
...
6. **Confirm coverage is clean** — re-run the report; it should show `missing: 0`
   ...
   ```bash
   ./scripts/translations/find-missing.sh
   ```
...
7. **Remove the payload.** Once the translations are applied and verified, delete
   ...
   ```bash
   rm -f scripts/translations/payload.json
   ```
...
- `sync.py` reports any missing key that has no entry in the payload as
  "still untranslated" — add it to `scripts/translations/payload.json` and re-run.
