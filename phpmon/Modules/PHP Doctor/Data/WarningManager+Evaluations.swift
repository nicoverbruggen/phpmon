...
extension WarningManager {
    ...
    func allAvailableWarnings() -> [Warning] {
        // Each warning is bound to its own local, then gathered into the array at the
        // end. Building the result from separate statements — rather than one large
        // array literal — keeps the type-checker from choking on a single huge
        // expression, which triggers a compiler crash under strict concurrency checking.

        // SYSTEM
        let rosettaWarning = Warning(
            command: {
        ...
        // The fix is only offered on a valid zsh shell. It's hoisted out of the
        // initializer below because an inline ternary producing an optional async
        // closure trips a compiler crash under strict concurrency checking.
        var helperPathFix: (() async -> Void)?
        if self.container.paths.isConfiguredShellValid && self.container.paths.shell == "/bin/zsh" {
            helperPathFix = {
                // Add to PATH
        ...
        let invalidShellWarning = Warning(
            command: {
                return !self.container.paths.isConfiguredShellValid
            },
        ...
        // HOMEBREW
        let missingTapsWarning = Warning(
            command: {
        ...
        let untrustedTapsWarning = Warning(
            command: {
        ...
        let expiredCertificatesWarning = Warning(
            ...
            url: nil,
            fix: {
                DomainListVC.show(certificateRenewalPrompt: .suppressed)
        ...
        return [
            ...
            xdebugModeWarning,
            missingTapsWarning,
            untrustedTapsWarning,
            missingConfigFilesWarning,
            expiredCertificatesWarning
        ]
    }
